...
class TestSolution(unittest.TestCase):
    ...
    def setUp(self):
        np.random.seed(0)
        aperture = np.random.randint(0,150,size=(100,100))
        pass
    ...
    def test_q1(self):
        ...
        np.testing.assert_allclose(aperture_statistics(aperture), (0,149))
        return
    ...
    def test_q2(self):
        ...
        np.testing.assert_allclose(aperture_mean(aperture), 74.5306)
        return
    ...
    def test_q3(self):
        ...
        np.testing.assert_allclose(roughness_coeff(aperture), 61.319589039718785)
        return

        
    def test_q4(self):
        # Instead, it's better to hardcode the solution if possible
        np.testing.assert_allclose(tortuosity(aperture), 49.970110174196435)
        return
        
    def test_q5(self):
        np.testing.assert_allclose(perm(aperture), 462.90086136333343)
        return
